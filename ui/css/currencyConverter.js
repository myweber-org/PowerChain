...
export { CurrencyConverter, formatCurrency };function fetchExchangeRate(baseCurrency, targetCurrency) {
...
function updateConversionResult(result) {
    if (!result) {
        return 'Conversion failed. Please check your inputs.';
    }
    
    const formattedOriginal = formatCurrency(result.amount, result.baseCurrency);
    const formattedConverted = formatCurrency(result.convertedAmount, result.targetCurrency);
    
    return `${formattedOriginal} = ${formattedConverted} (Rate: ${result.rate.toFixed(4)})`;
}
