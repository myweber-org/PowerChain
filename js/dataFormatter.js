...
export { formatDateWithTimezone };function formatDateToISOWithOffset(date) {
...
export { formatDateWithTimezone };function formatDateWithTimezone(date, includeTime = true) {
    if (!(date instanceof Date)) {
        date = new Date(date);
    }
...
module.exports = formatRelativeTime;function formatRelativeTime(dateString) {
  const date = new Date(dateString);
  const now = new Date();
  const diffInSeconds = Math.floor((now - date) / 1000);
...
export default formatRelativeTime;function formatDate(dateString, locale = 'en-US') {
    const date = new Date(dateString);
...
export { formatDateToISOWithTimezone };